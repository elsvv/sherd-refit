//! What a front end does about `--backend` and `--gpu-adapter` (D §6.8, D §9): the CLI and the
//! desktop app (A §3.6) resolve it here, identically.
//!
//! The whole of the GPU crate's surface that a front end needs in order to *choose* a backend is
//! used from this crate and nowhere else, so a build with `--no-default-features` — no
//! `sherd-gpu`, no wgpu, no `naga` — differs from the default build in this file alone. That is
//! D §2's "`sherd-gpu` is an optional feature of `sherd-cli`", carried down one level so that
//! `--no-default-features` on a front end reaches the whole tree below it.
//!
//! # What `--backend` means
//!
...
/// which.
#[must_use]
pub fn selftest_lines(adapter: Option<&str>) -> Vec<String> {
    #[cfg(feature = "gpu")]
    {
...
pub struct Resolved {
    ...
    #[cfg(feature = "gpu")]
    pub executor: Option<&'static sherd_gpu::GpuExecutor>,
}
...
#[cfg(feature = "gpu")]
pub fn resolve(backend: Backend, adapter: Option<&str>, memory: Option<f64>) -> Result<Resolved> {
    use std::sync::Arc;
...
#[cfg(not(feature = "gpu"))]
pub fn resolve(backend: Backend, _adapter: Option<&str>, _memory: Option<f64>) -> Result<Resolved> {
    match backend {
        Backend::Gpu => {
