...
| fracture BVH (R §6.1) | **measured** (task H3, `synthetic_20`, 3.66 M working-mesh faces): **1.2 B per working-mesh face** — 2.3 MB at 183 k faces | ≈ 0.4 GB at 170 × 200 k faces | **released after matching** (audit §B.3): its one reader is R §6.1's point-to-surface distance |
| full BVH (R §6.4's penetration) | **measured** the same way: **12.9 B per working-mesh face** — 23.6 MB at 183 k faces, six times the 4 MB this row used to guess | ≈ **4 GB** at 170 × 200 k faces | **released after the last `try_place`** (audit §B.3). There is no LRU and this row used to claim one: during all-pairs matching every fragment's tree is live, so an LRU smaller than the collection thrashes and one as large as it is what the `OnceLock`s already were. A release point is the instrument, not an eviction policy |
