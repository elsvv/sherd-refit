...
| peak RSS, 170 fragments | 9 × 384 MB workers, but 16 GB exhausted on large scans | ≤ 6 GB (≤ 3 M faces/scan); ≤ 10 GB (10 M) | + ≤ 1 GB GPU | **2 715 MiB** worst of six runs, the process peak `/usr/bin/time -l` reports — **met, at 44 %** of the row. (`report.json`'s own 100 ms sampler reads the same six 0–70 MiB lower, worst 2 647 MiB; §10.3 says which instrument each of its figures is.) Under `--memory-budget 4.83` the semaphore never blocked once |
...
device's occupancy on `synthetic_20` went from at best 41 % to 68 % **as G3 left it — a figure
phase 2e then halved on purpose and no summary said so** (V6-D6; G4's `MAX_QUERIES` paragraph
below states the halving, 10.2 → 5.8 s outstanding, but every place that quotes the phase's
occupancy quotes G3's 68 %). Re-measured at task W on the tuned tree, `synthetic_20`, three
interleaved warm runs: **5.44 s outstanding of an 11.34 s matching stage — 48 %** (48.0/48.3/48.6 %
over the three), and on `pot_H` 0.70 s of 6.37 s, **11 %**, because not one ICP rung of that
collection reaches the device and its 1.11× is the coarse kernel alone. Less device time for a
faster stage is this section's conclusion, not a contradiction of it. **Phase 2e (task G4) tuned the
...
`max(1, min(⌈threads/2⌉, cores + 1 − threads))`. **The outer `max(1)` is deliberate and was
undocumented until task W** (V6-D5): past `--threads > cores` the inner expression is zero, and a
slack of zero would leave *no* worker to occupy the cores while one waits on the device — the
single thing this pool exists for. One is the floor, so a `--threads 12` run on ten cores gets a
pool of 13 rather than 12. It never fires at the default (`--threads 9` on ten cores gives 2 either
way), and §7's thread-count row is what keeps it from mattering to a result. The two changes together are worth **1.10×** of the
...
|---|---|---|---|---|---|---|---|---|---:|---:|
| terracotta | 2.3e-4 | 2.3e-5 | 0 | 1.3e-6 | **2.113** | **2.0e-1** | **1.6e-3** | **7.7e-4** | 0 / 40 | 1 |
| pot_A | 2.6e-2 | **4.3e-2** | 0 | 9.7e-5 | 1.1e-2 | **1.8e-2** | **6.0e-4** | **1.5e-4** | 0 / 40 | 1 |
| pot_B | 4.4e-2 | **7.8e-2** | **3.1e-3** | **1.8e-3** | 1.6e-2 | **1.7e-2** | **5.2e-4** | 8.4e-5 | 0 / 40 | 1 |
| pot_C | **9.1e-2** | **1.3e-1** | **1.9e-3** | **2.6e-3** | **5.030** | **7.274** | **3.5e-3** | **2.3e-3** | 4 / 40 | 1 |
| pot_G | 1.7e-4 | 5.6e-4 | 0 | 9.4e-7 | 2.8e-2 | **7.0e-2** | 8.3e-5 | 3.2e-5 | 1 / 40 | 1 |
| pot_H | 1.9e-4 | 3.4e-4 | 0 | 1.0e-6 | **1.466** | **2.330** | **3.3e-3** | **9.9e-4** | 1 / 40 | 1 |
| synthetic_20 | 5.0e-4 | 4.6e-4 | 0 | 6.3e-7 | **3.8e-1** | **2.7e-1** | **4.0e-4** | **6.6e-4** | 0 / 40 | 1 |
| slab | 7.9e-5 | 8.3e-6 | 0 | 4.3e-7 | 4.0e-2 | 4.2e-3 | 0 | 6.0e-7 | 0 / 10 | **0** |
| **inside the row** | 7/8 | **5/8** | 6/8 | 6/8 | 4/8 | **1/8** | 2/8 | 3/8 | 8/8 | 1/8 |

Read with it, three things that were stated wrongly or not at all before task W:

* **The translation column above is the origin-referenced one, which is the row §10.2 actually
  writes** (V6-D4). Every earlier summary of this phase quoted the displacement read *at the
  cloud* — a real and useful number, introduced in §10.4 layer 3 as sitting *beside* §10.2's, not
  in place of it. At the cloud stage 1 is inside on 8 of 8 and stage 2 on 4 of 8; at the origin,
  which is the row, 5 of 8 and 1 of 8. These scans sit 100–150 units from the origin, so the two
  readings differ by more than rounding and the conservative one is the row's.
* **The tail is the kernel's on four sets, not one** (V6-D3). This section used to attribute the
  stage-2 tail to "the ladder amplifies any `f32` input" on three sets and to the kernel on "one
  set, one candidate of forty, at 2.1°". That was read off the *columns*: the worst `ctrl` beside
  the worst `deg`. Task W measures it **per candidate** — the control chain is per candidate, and
  so is the exclusion — and the two are not the same candidate. After every candidate whose own
  `f64` control is outside the row has been excused (4 on pot_C, 1 on pot_G, 1 on pot_H, none
  anywhere else), terracotta, pot_C, pot_H and synthetic_20 are **still** outside the 0.05° row,
  at 2.113°, 5.030°, 1.466° and 0.380°, on candidates whose ladders are functions of their input.
  The ladder is a real effect and it is not the explanation.
* **Excusing the chaotic candidates changes no set's verdict.** The `chaotic` rows are inside
...
final pose gap between them is 7.0e-2° and 2.9e-4 t — inside §10.2's `refine` row, which is what a
placed pose is held to — and two GPU runs are byte-identical (with the adapter to itself; see §7's
last row).

**What would close it, and what would not.** Task W's own experiment (its note, §2) tried the
obvious remedy and rejected it on measurements: double-single accumulation of the 27 JTJ/JTr
partials and of the residual sums, and then the 6×6 solve in double-single as well, both written
through `fma(-1.0, b, a)` because Metal's fast math folds every other spelling of a compensated
subtraction to zero. Neither moved the kernel-attributed sets — terracotta stayed at 2.113° and
synthetic_20 at 0.380° to four digits — while costing up to 1.13× and 1.22× of device time. The
`f32` loss is not in the accumulation; the remaining candidates are ladders whose *rung count*
diverges, and the fix, if one is wanted, is a rung the two sides cannot diverge in rather than more
precision in the one they have.
...
| derived per fragment (FN, A, C, grids) | ≈ 6 MB | ≈ 1 GB if all resident | the 400 MB this row used to quote was an estimate for 10 M-face scans and has never been reached on a development set. The `MatchData` LRU it also described was removed in task H2 (§5 step 3) |
| fracture BVH (R §6.1) | **measured** (task H3, `synthetic_20`, 3.66 M working-mesh faces): **12.5 B per working-mesh face** — 2.3 MB at 183 k faces | ≈ **0.4 GB** at 170 × 200 k faces | **released after matching** (audit §B.3): its one reader is R §6.1's point-to-surface distance |
| full BVH (R §6.4's penetration) | **measured** the same way: **129 B per working-mesh face** — 23.6 MB at 183 k faces, six times the 4 MB this row used to guess | ≈ **4.4 GB** at 170 × 200 k faces — **an extrapolation no collection has reached** (task A1, §10.3): the face count is what is wrong with it, not the constant. Measured working-mesh totals over 164 fragments are 6.48 M, 9.01 M and 4.66 M faces (means 116 k, 55 k and 28 k) against the row's assumed 34 M, so the tree is **0.60–1.16 GB** on real collections of this size | **released after the last `try_place`** (audit §B.3). There is no LRU and this row used to claim one: during all-pairs matching every fragment's tree is live, so an LRU smaller than the collection thrashes and one as large as it is what the `OnceLock`s already were. A release point is the instrument, not an eviction policy |
...
| **peak RSS** | | **≈ 3–6 GB** (≤ 3 M faces), **≤ 10 GB** (10 M-face scans, 3 preprocessing workers) | **the largest measurement is §10.3's: 2 715 MiB over 164 fragments** at the final acceptance (task A1, `/usr/bin/time -l`), 44 % of §1's row. The rest of this cell is the development-set measurement it was written from — the largest development set, warm, previews and meshes on: **1.9 GiB** after E2 against 1.6 GiB before it, the rise being E2's parallel writers and its `MatchData` cache (`notes/2026-09-07-e2-tuning.md` §9). Task H3 put the number **per stage** in every run — `report.json`'s `memory` block beside `timings`, sampled at 100 ms — so this row stops being the only measurement of it: on `synthetic_20` the peak is matching's, 1.5–1.7 GiB warm, and the release above returns **about 200–275 MiB** at the moment it happens. That figure is an *order*, and it is stated here and nowhere else (V7-D3): six warm runs across two tasks measured 201, 205, 231, 272, 275 and 276 MiB, so a re-run that lands outside any narrower band would be reporting the allocator's history and not a regression |
...
| `--constraints FILE` | roadmap item 3's operator constraints (§11). **Built in task T2**: `constraints.json` v1 with audit §D.1's four lists — `must_not_join` removes the pair before matching and R §8 refuses any candidate for it, `must_join` matches the pair with R §8.1's budget and promotes its best probable candidate to confirmed (or, with a 4×4 `pose`, skips matching and pins it), `same_object` and `different_object` are step 10's evidence and the second already vetoes a join. Every name is validated against the collection and an unknown one **fails the run**; a pair in two lists, a non-rigid pose and an unsupported `version` are errors too. A constraint never edits a score, and `report.md`'s `## Constraints` lists every line of the file and what it did |
| `--review-images` | roadmap item 3: `review/<a>__<b>.png` for **confirmed and probable** joins. **Built in task T2**: `render::render_pair` on R §11.5's own splat renderer — three views, A grey and B orange, `verify::seam_cells`' voxels white, B's fracture samples coloured by R §6.1's own distance to A's fracture surface, a caption with the scores, the band, the margin and the seed. One `rng(seed)` per image, so two runs write the same bytes; `report.md`'s per-fragment index links them |
| `--probable-top N` (default **100**) | how many rows of the **probable** band `report.md` shows, best `seam · tight` first; `0` shows the whole band. **Built in task C** (`tiers::probable_shown`): one ranked list shared by `## Probable joins`, the per-fragment index and `--review-images`, so a row, its picture and the count cannot disagree. When the band is longer than `N` the section says so — *"100 of 2761 shown"* — and names where the rest is. The number is A1 §3.2's measurement and not a preference: on `mixed_all` the band is 2 761 rows holding 61 true joins, and the **first hundred hold four fifths of them**, one in two of those hundred being real. **`report.json` is never cut**, so nothing a run found is lost, and a shorter band is a shorter `review/` directory: 123 images instead of 2 784. A band no longer than `N`, which is every development set, is printed exactly as it was before the flag |
| `--objects on\|off` (default `on`) | roadmap item 4's object separation (§11). **Built in task O1** (audit §D.2): every assembled group is reported as an **object** with the median and MAD its members agree on — wall, shell radius, fracture roughness, rim diameter where R §3.2 flagged one — and the members that consensus does not fit; a **confirmed** join between two groups merges them, under the penetration test across both groups and consistency with every cross-group join the gate admits, replacing R §8's `MergesGroups`. **No feature vetoes anything**: audit §D.2's own rule is AUC > 0.800 and task M1 §4 measured the best at 0.740 on the sets with real object ids, so `--object-demote` ships empty and every number reports. `--objects off` reproduces task T2's outputs byte for byte |
| `--object-demote LIST`, `--object-k-mad K` (3), `--object-min-members N` (3), `--object-merge on\|off` (`on`), `--object-disagreement on\|off` (`off`) | the four knobs of the object pass. `--object-demote` is the shortlist a `k·MAD` deviation may demote a join on — **empty**, on M1 §4's table. `--object-disagreement` is audit §D.2 (b)'s mutual-disagreement demotion, **off** on the measurement: the confirmed tier has 0 false joins over 8 sets × 5 seeds, so a contradiction has none to catch, and switching the arm on takes `mixed_ABG` from 8 / 7 / 7 / 7 / 9 confirmed joins at seeds 0–4 to 2 / 3 / 2 / 4 / 0 — 27 of 38 demoted, every one correct (re-measured on this tree by task C; the numbers live in `ObjectParams::default`'s doc, and task O1's 11 → 2 was measured before task G's watertight refusal moved the denominator) |
| ~~`--export-glb`~~ | ~~additionally write `assembly_<k>.glb` for the desktop viewer~~ — **superseded** by the museum's request of 2026-09-11 (`crates/sherd-core/src/export/`): every `run` writes `scene.glb` — one named node per fragment carrying its `transforms.json` matrix over a simplified copy of its original, group nodes that only translate so the groups R §8.2 centred on one origin stand apart — and `viewer.html`, the same scene in a self-contained WebGL2 page that shows a fragment's name under the cursor; plus `transforms.csv`, `joins.csv`, `matrices/<name>.txt` and a Russian `README.txt`. All additive: `transforms.json` and every `placed/<name>.ply` written are the bytes they were |
| `--no-viewer`, `--viewer-faces N` (600 000) | skip `scene.glb` and `viewer.html`; the faces they hold over the whole collection, shared by surface area, 2 000–60 000 a fragment |
| `--cache-dir DIR` | R §3.7's fragment cache as `<DIR>/<name>.sherd`, **opt-in** since 2026-09-11: a finished run's cache serves only a re-run of the same collection, and the result folder holds results. `tools/quality_gate.py` passes it; `bench` and `segment` keep `<out>/cache` |
| `--placed-all`, `--merged-meshes` | R §11.4's own outputs, now opt-in: `run` writes `placed/<name>.ply` only for the fragments an assembled group placed (an unassembled fragment's copy is its original, recentred — 106–127 of 164 on `mixed_all`), and `assembly_<k>.ply` (a nameless repeat of `placed/`) only on request. `report::write_placed_meshes`, which parity runs, is unchanged; `run` calls `write_placed_selected` |
...
| `mixed_all` (12 612 pairs) | ≤ 2 h | ≤ 30 min | **run once, at the final acceptance** (task A1): **30.3 min** CPU cold, **30.3 min** warm — inside, 4.0×. **Re-measured under task S3's rule at the second acceptance** (task A2, 2026-09-11): see the A2 table below. **Re-measured again under task R1's rule** (task R1, 2026-09-11): **39.5 min** cold — inside, **3.0×**; with `--tier-agree-seeds 2`, which matches and tiers the collection twice, **78.6 min** — inside, **1.5×** |
| `synthetic_mix3_60` (1 584 pairs) | ≤ 15 min | ≤ 5 min | **run once, at the second acceptance** (task A2, 2026-09-11): **2.2 min** CPU cold, **2.0 min** warm — inside, 6.8×. Task S1's collection: three vessels, 58 fragments, real photographic colour on every one |
...
read, only R §3.5's three sampled arrays redrawn, R §3.7). Peak RSS in the table is the **process
peak from `/usr/bin/time -l`**, and every figure in this section is that instrument unless it says
otherwise; `report.json`'s own 100 ms sampler (§8) reads the same runs 0–70 MiB lower, because it
samples a curve `/usr/bin/time -l` takes the maximum of:
...
the six runs is 2 715 MiB — 44 % of §1's 6 GB row** (that is `/usr/bin/time -l` on synthetic 170
warm; `report.json`'s sampler puts the worst of the six at 2 647 MiB, and task V9's re-run of
synthetic 170 seed 0 measured 2 751 MiB, which is the allocator's history rather than a change).** Cold minus warm is the `preprocess` stage and
...
**What the projection got right and what it could not see.** The **wall** predictions were close —
−12.5 %, +8.8 % and +8.2 % on the three collections — and they were close because two errors ran
against each other: E2's per-pair cost was **36 % low** on synthetic 170 (0.523 core-s projected
against **0.71** measured; **+10.5 %** on synthetic 60 and **−4.7 %** on `mixed_all`, whose 0.870
projection is the closest of the three), and the pool returns **8.4–8.5×** of ten cores on runs of
this length rather than the 6.43× the projection assumed. The matching stage itself came in at
17.9 min (projected 17) and 20.5 min (projected 28). What the arithmetic could not see is a stage
that did not exist when it was written: **R §6.5 accepts 8 444 candidates on `mixed_all`, the tier
pass of roadmap item 3 probes every one of them, and that costs 580 s — 32 % of the run.** On
synthetic 170 the same stage is 16.8 s, because **464** candidates were accepted there (150
confirmed + 314 probable in its `report.json`; the run's log line reads `candidates=54045
accepted=464`). The tier's cost is linear in the *accepted* list, not in the pair count: the list is
**eighteen times** longer on a real ten-pot collection than on a synthetic one-pot collection of the
same size, and each candidate on it costs about twice as much — **69 ms against 36 ms**. §12's
step-8 risk column asked that the probes stay a few per cent of a pair; per candidate they are, but
the collection-level share is a third of the run.
...
**Measured again at the second acceptance, task A2 (2026-09-11,
`notes/2026-09-11-a2-acceptance.md`), because task S3 replaced the rule step 12 judged.** Same
command, same flags, same machine, commit `1e34e79`; `synthetic_mix3_60` (task S1's three-vessel
coloured collection, 58 fragments, 1 584 pairs, matched for the first time) and `mixed_all` at seeds
0 and 1. `synthetic_60` and `synthetic_170` were **not** re-run — their rows above are A1's, made
under M1's rule, and this note does not claim otherwise.

| set | seed | cache | **real** | user/real | preprocess | matching | **tiers** | refine | **peak RSS** | CPU gate | margin |
|---|---:|---|---:|---:|---:|---:|---:|---:|---:|---|---:|
| `synthetic_mix3_60` | 0 | cold | **131.8 s** (2.2 min) | 7.5× | 16.31 | 94.12 | **19.96** | 1.17 | 1 749 MiB | ≤ 15 min | **6.8×** |
| `synthetic_mix3_60` | 1 | warm | **118.2 s** (2.0 min) | 7.5× | 0.59 | 96.68 | **19.41** | 1.31 | 1 827 MiB | ≤ 15 min | **7.6×** |
| `mixed_all` | 0 | cold | **2 539.1 s** (42.3 min) | 7.8× | 10.78 | 1 323.15 | **1 203.74** | 0.82 | 1 413 MiB | ≤ 2 h | **2.8×** |
| `mixed_all` | 1 | warm | **2 626.2 s** (43.8 min) | 7.7× | 1.22 | 1 384.71 | **1 238.79** | 0.73 | 1 109 MiB | ≤ 2 h | **2.7×** |

**Measured a third time at task R1 (2026-09-11, `notes/2026-09-11-r1-rule-on-real.md`), because
task R1 replaced the rule again and added a mode that costs a second run.** Same command, same
flags, same machine, seed 0 into an empty work directory both times:

| set | run | **real** | user/real | preprocess | matching | **tiers** | **tier_agree** | **peak RSS** | CPU gate | margin |
|---|---|---:|---:|---:|---:|---:|---:|---:|---|---:|
| `mixed_all` | the shipped default | **2 369.4 s** (39.5 min) | 8.5× | 10.17 | 1 233.79 | **1 124.13** | — | 1 986 MiB | ≤ 2 h | **3.0×** |
| `mixed_all` | `--tier-agree-seeds 2` | **4 717.0 s** (78.6 min) | 8.5× | 10.01 | 1 229.98 | **1 137.60** | **2 338.49** | 2 237 MiB | ≤ 2 h | **1.5×** |

**Task R1's rule costs nothing: it only sorts.** Its three added conjuncts read evidence a run with
the tier pass on already computes, so the stage clock is A2's on a quieter machine — the tier pass is
**47.5 %** of the stage time, which is A2's 47 %. What *does* cost is the new **mode**:
`--tier-agree-seeds 2` matches and tiers the whole collection a second time at another seed and
confirms only what both runs put in the same place, so it is **1.99×** the default and still 1.5×
inside the gate. It is off by default for that reason.

**Runtime and memory are still met, with less margin than A1 had.** `mixed_all` is **39.6 % slower
than at task A1** — `matching` +7.8 % because the wide rival is read off R §5.6's full list inside
`match_pair_wide` before `keep` truncates it, and `tiers` **+107 %**, from 580.5 s to 1 203.7 s,
because every accepted pair's whole R §5–§6 search is run twice more. **The tier pass is now 47 % of
a `mixed_all` run, against the 32 % this section recorded for A1**, and a paired run with
`--tier-research 0 --resample-seeds 0` prices the re-search alone at **569 s — 43 % of the matching
stage**. Peak RSS fell rather than rose (1 413 MiB against A1's 1 754), and the memory semaphore
never blocked. The 2 h gate holds at 2.7–2.8×; a collection twice this size would not.

**Quality: the coloured collection is clean, `mixed_all` still is not, and the numbers moved in both
directions.** `synthetic_mix3_60` confirms **38 and 41 joins, every one correct**, cross-object 0 and
group purity 1.000, at a confirmed recall of **0.299–0.323** — and its probable band is **23 and 21
rows that are 100 % true joins**, which no collection of this project had done before. `mixed_all`
confirms **75 joins over the two seeds, 62 correct against A1's 41 (+51 %) and 13 false against
A1's 10**; confirmed recall rises 0.066/0.076 → **0.111/0.104**, tier precision 0.804 → 0.827,
cross-object joins at seed 1 fall 3 → **2** and group purity rises 0.930 → **0.966**, and fragment
accuracy rises 21.8/23.2 % → **33.8/33.1 %**. The two bands together hold **80 and 84 correct joins
on both tasks' runs, to the join**: every join the confirmed band gained came out of the probable
band below it, which is what a confidence tier does. **Step 12's prohibition is breached at both
seeds** and every one of the 13 false joins is on the **margin** arm with `support` 0 — the same
sentence A1 §3.4 wrote about its ten.

**What this run refutes is a threshold, and the note said which one.** Task S3 chose `RIVAL_FAR_T`
= 5 t because a false join's second placement sat at a median of 2.98 t against a correct one's
19.83 t over the 45 development runs (AUC 0.806), and S3 §9 item 3 asked for that to be re-read on a
second body of real sherds. Re-read on `mixed_all`, the two medians are **15.85 t (false) against
18.04 t (correct)** at seed 0 and **14.19 t against 12.59 t** at seed 1 — no separation at all, and
at seed 1 the false joins are the *further* of the two. `pot_H`'s wrong-pose family is a sherd slid
three walls along a straight break; `Pot_I`'s is a sherd that fits a genuinely different place on
the pot. The constant was fitted on the first and this collection is the second. **What the run
confirms is the other conjunct**: dropping the re-search takes the band from 13 false joins to 15
and the cross-object count from 2 to **4**, so S3 §9 item 2's proposed simplification is refused
here. `support ≥ 1` alone remains the only clean rule on this collection — 16 confirmed joins over
the two seeds, all correct, at 2.8 % recall against the shipped rule's 10.8 % — which is A1 §3.5's
trade, unmoved and again **measured and not applied**. A2 §4.2 is the full rule table.
...
3. **Kernel cross-checks** (`sherd_gpu::crosscheck`, moved there from the CLI in task H2 so that
   ...
   **Phase 2b filled the coarse and ICP rows and found the same trap from the other side**: on
   ...
   them all to the CPU and the harness compared the CPU with itself. Phase 2b's answer was to force
   the kernels on by default, behind a `--policy` flag that asked for the thresholds a run applies;
   task H2 turned that round — the policy is the default and `--force-device` is the flag — so
   read the paragraph below, not this one, for what the harness does today. What survives from 2b
   unchanged is that **every row that compares a kernel reports how many of its calls reached the
   device**. It also gains
   ...
   **Task W made the exit code the criterion and nothing else** (V6-D2). A candidate is excused
   from the pose rows for one of three *measured* reasons, each counted and bounded in a row of
   its own:

   * its **control** is already outside §10.2's pose tolerances — the ladder amplifies any `f32`
     input, so a worst case over it measures the ladder. One extra ladder, so always applied;
   * or C2's twelve one-ULP neighbours move its own CPU answer out of the row. Thirteen extra
     climbs, so behind `--chaos`. **The run without the flag is therefore the stricter of the
     two**, not the laxer, because it excuses fewer candidates;
   * or its **correspondence set stands on the boundary** — `crosscheck::boundary_batch`, task F.
     The first two perturb the pose the ladder *starts from*, and §A.2.2 of the audit names a
     discontinuity no starting pose reaches: a source point closer to the correspondence radius
     than the two executors' own coordinates are to each other, at any iteration of any rung, is
     in on one side and out on the other, and from there the two sets differ. The probe re-climbs
     the stage-1 ladder on the CPU with every rung's radius moved by ±1 `f32` resolution of its
     clouds (`spread · f32::EPSILON`, an absolute distance) and excuses a candidate whose own
     answer then leaves the pose rows. Two extra ladders, so always applied, like the control.
     **Stage 1 only**: that with the coarse score is what a run puts on the device, so it is what
     the criterion is read over; stage 2's rows are `cpu by policy` and have nothing to excuse,
     and under `--force-device` they stay exactly as task W measured them.

   The `ctrl` row is reported over *every* candidate with no tolerance of its own — gating the row
   that defines the exclusion would gate the same fact twice — the two `icp sN chaotic` rows carry
   §10.2's own per-pair bounds (0.06 and 0.4; four pairs are not a dump, and the per-dump bounds
   are stated over tens of thousands of candidates), and `icp s1 bound` carries the stage-1 number
   again for the third probe. The three counts are kept in separate rows because their bounds come
   from different measurements: C2 §5's is a one-ULP probe of `f64` and task F's is the `f32`
   resolution the two executors share. `gpu-check` exits non-zero **exactly when** some row is
   over its tolerance, which is exactly the criterion §12's 2b row states. Read at the production
   policy that is **all eight development sets** since task F (seven of eight at task H2, one of
   eight when every rung was forced onto the device); the `bound` row's own worst pair over those
   eight sets is 2.0e-2 of 6.0e-2, so the new excusal is bounded well away from anything that
   ...
   **The `distance` and `inside` rows read `delegated` on every set, permanently** (V6-D9). Phase
   ...
   half of this layer's table has nothing to compare. That is a label and not a zero, and it is
   recorded here so that the table is read with it in view rather than as eight rows of which four
   silently pass.
...
7. **Quality gate** (`tools/quality_gate.py`, task H4): the eight development sets — terracotta,
   ...
   seed sweep. **Measured on the shipped defaults: 45 runs in 15.5 min** on this machine
   (930.4 s, task R1; 988.0 s at task A2) against the 25-minute budget the hand-over was made
   under. The set list grew
   from eight to nine when task S1 added `synthetic_mix3_24`, and the tier pass grew again when
   task S3 added the re-search; before both, five runs of the same **forty** across tasks O1, G,
   A1, V8 and V9 measured 602.2, 610.9, 611.7, 614.9 and 602.6 s, each one's own
   `meta.wall_total_s` in `quality.{md,json}`. Cold caches, the work directory of a set reused
   across its five seeds so only the first pays for the fragment cache (R §3.7: another seed
   recomputes R §3.5's three sampled arrays and nothing else). **Roughly two fifths of that is the
   tier pass**: those forty runs under `--tiers off` took 412.2 s (task V8) and 459.8 s
   (task T1), and 421.7 s is task H4's figure from before the tier pass was the default.
   ...
   * The **gate** is what R §13 states as a *prohibition*: its last row — cross-object joins 0 and
     ...
     terracotta's decision row. A failure of one of those exits non-zero. All three prohibitions
     hold at all five seeds on this tree, **and since task S3 the run exits 0**: the terracotta's
     *recall* row — ten of ten (seed, join) slots — is the last one that was short, and the wide
     rival closed it (task S3 §4.2, re-measured by task A2 §8). A red gate here need not be a
     breached prohibition, and the run's last line says which of the two it is.
   ...
   * *Step 10, `mixed_ABG`, under `--tiers on`.* The **prohibition** — cross-object joins 0 and
     ...
     **confirmed and probable bands together**: the audit's 12 counts the joins R §6.5's own
     ...
     seed (7 / 7 / 8 / 6 / 10 under task R1's rule; 8 / 7 / 7 / 7 / 9 under M1's, 12 / 9 / 8 / 7 / 12
     under S3's) and deliberately not gated: a floor fitted to what the tier reaches
   ...
   * *Step 8, the terracotta.* "Both joins confirmed at seeds 0–4" is **ten** (seed, join) slots and
     the gate asks for ten. **This tree reaches ten** (2/2/2/2/2, task R1 §7). It is also the row
     that decided task R1's rule: every single-run rule its table found with zero false confirmed
     joins on `mixed_all` asks the second placement to be 25–30 wall thicknesses away, and the
     terracotta's own two joins have rivals at 9.98–26.21 t, so at 25 t this row reads **two** of
     ten and at 30 t **none**. It reached nine from
     task G until task S3: at seed 4 the pair 021–094 came back from R §5.7 as a single placement,
     so M1's margin arm had nothing to beat and a three-fragment chain gave the support arm no
     second path. The gate used to ask for nine, a constant fitted to that measurement — a gate
     that cannot fail; it was changed to ask for ten, to name the missing slot, and to say that the
     failure was a *recall* row and not a prohibition. What closed it is the algorithm change task G
     said it would need: task S3's wide rival reads R §5.6's full list, which finds that pair a
     second placement where the kept list has none — `notes/2026-09-11-s3-recall.md` §4.2,
     `notes/2026-09-10-g-tiers-findings.md` for the two rules task G measured and rejected.
   ...
   **`--score-only NAME RUNDIR` scores a run this script did not make** (task A1). §12's small-set
   rule matches nothing above 27 fragments before step 12, so `synthetic_60`, `synthetic_170`,
   `mixed_all` and (since task S1) `synthetic_mix3_60` are deliberately **not** in the script's
   `SETS` and no ordinary run of it can reach
...
|---|---|---|
| 3 confidence tiers, review images, constraints — **built**, tasks T1 (`notes/2026-09-09-t1-tiers.md`) and T2 (`notes/2026-09-09-t2-review-constraints.md`) | `tiers.rs` holds `Tier`, the strict `Thresholds` set and the five probes; `Params::tiers` carries the set and `--tiers on\|off` switches it; R §8 assembles from `Gate::Confirmed`. `render::render_pair` and `review.rs` write `review/<a>__<b>.png` behind `--review-images`; `assembly/constraints.rs` is `constraints.json` v1 and `assemble_under` is R §8 under it. `report.md` gains Confirmed / Probable / Rejected, the per-fragment index with its `image` column, and `## Constraints` | the types and the file format were phase 1's; both behaviours have an off switch that reproduces the previous step's outputs byte for byte |
| 4 object separation — **built**, task O1 (`notes/2026-09-09-o1-objects.md`); measured on `mixed_ABG`, whose baseline both implementations set in §10.3: port 3 cross-object joins at purity 0.864, reference 1 at 0.706, fragment accuracy 58.3 % on both | `fragment/features.rs` computes audit §D.2's table in R §3's preprocessing and `CACHE_VERSION` 6 first carried it (7 since task S2's colours); `objects.rs` holds `FeatureKey`, `GroupFeatures` (median and MAD per feature), the consensus arm, audit §D.2 (b)'s mutual-disagreement arm and the object report; `Params::objects` carries the rules and `--objects on\|off` switches them; `Grouping::merge` and `greedy::Inputs::try_merge` are audit §D.2 (c)'s group merge, and `report.md` gains `## Objects`. **Which features veto is a measurement and the measurement said none**: M1 §4's best AUC on a collection with real object ids is 0.740 against audit §D.2's own 0.800, so the shortlist is empty and the consensus reports. **Colour is the one feature above that bar and it still vetoes nothing** (tasks S1, S2, S3, A2): `frac_lab_a` — the red–green axis of the clay a break shows — separates same-object from different-object pairs at **AUC 0.985 / 0.990** on the two coloured collections task S1 built, and `Features` carries the shell colour and the fracture colour separately (Lab mean, Lab MAD, a 4×4×4 Lab histogram each, `CACHE_VERSION` 7). It vetoes nothing because there is nothing for it to veto: on both coloured collections R §6.5 accepts **zero** cross-object candidates, so `--object-demote frac_lab_a,shell_lab_a` demotes nothing at any seed; without `objects::COLOUR_MAD_FLOOR = 3.0` the same flag costs 11 correct confirmed joins on `synthetic_mix3_24` and 13 on `synthetic_pingsdorf_20` and still removes no false one; and the one collection whose confirmed band *has* false joins, `mixed_all`, is SfS++ OBJs with no vertex colours at all. Every accepted candidate records a `colour_agreement` (`frac_delta_e`, `shell_hist`), printed and read by no rule | `Features` struct and cache slots (empty in phase 1), the PLY colour path, `assembly/consistency.rs`'s tolerances, which the merge and the disagreement arm both read rather than inventing their own |
...
**Phase 3 is replaced by the audit's amended plan §E** (2026-09-09, task H4). The three rows this
table used to carry — 3a `pyo3`, 3b the Tauri app, 3c packaging — were written on the assumption
that §13's question 7 would be answered at the end of 3a. It was answered instead at task H4's
first commit, which strikes 3a and moves the app behind the algorithm work rather than beside it.
What stands in their place are the audit's **steps 7–12** — measure → tiers → review images and
constraints → object separation → an optional spike → final acceptance — with steps 1–6 of the same
plan already executed as tasks H1–H4. **Their effort column is in agent-hours for one Opus
executor including its own verification, not in engineer-weeks**, because that is how steps 1–6
were done and how these are estimated.

Rules that hold for every one of steps 7–12:

* **The small-set rule.** Nothing above 27 fragments is *matched* before step 12. The development
  sets are the terracotta (4), pots A/B/C/G/H (7–11), `mixed_ABG` (24) and `synthetic_20` (20);
  `mixed_all` (164) and `synthetic_170` are matched exactly **once**, at step 12. There are two
  stated exceptions, both **preprocessing only** and both linear in the fragment count: step 7's
  per-fragment feature pass, and step 11's segment matching, which reads breaklines and never
  forms a pair hypothesis. A step that needs a large collection for any other reason is a step
  that has to be re-planned.
* Every step ends with the standing gates: `cargo test --workspace` in debug **and** release, the
  two `--no-default-features` commands, `cargo clippy --workspace --all-targets -- -D warnings`,
  `cargo fmt --check`, `parity --stage all` in both modes on the eight dumps (frozen at 23 804 / 0,
  §10.4 layer 5), `pytest -q`, and the byte-identity check of terracotta, pot_A, pot_H and
  `synthetic_20` against the previous step **wherever the algorithm did not change**, with every
  difference the step's own brief names listed and no others.
* Quality is `tools/quality_gate.py` (§10.4 layer 7) — `evaluate.py` on the ground-truth sets at
  seeds 0–4 — and never a fixture dump: after the hand-over there is nothing on the Python side
  of a new stage to compare against.
* Outputs under `output/<task>/`, one note per step in `docs/superpowers/notes/`, and a step that
  changes a row count of the frozen parity harness says why in its note.

**What step 7 starts from.** The first five-seed sweep of the *port* — §10.4's layer 7, run at task
H4 — found its spread wider than the reference's on three sets of seven: `pot_A` 0.750 at seed 2,
`pot_B` 0.778 / 0.857 at seeds 2 and 4, `pot_H` 0.000 at seed 2. Two of those are false joins in
substance rather than lost recall: `pot_B` accepts the *same* non-adjacent pair (pieces 02 and 07,
145° and 39.6 t from the truth) at two of five seeds, and `pot_H`'s seed 2 keeps no correct join at
all while two of its wrong-pose joins miss by 0.56 t and 0.57 t against `evaluate.py`'s 0.5 t. R
§13's prohibitions — cross-object 0, purity 1.000, pot_G's rule, the terracotta's two joins — hold
at every seed of every set. That is the material steps 7 and 8 are for: a tier that a `pot_B` seed-2
join cannot enter, and a margin that says why.
...
| 2b, task G2 | done: `kernels/coarse.wgsl` (§6.5, one workgroup per hypothesis, an integer count out) and `kernels/icp.wgsl` (both estimators, every iteration of a rung in one dispatch, the shifted frame with §7's correction, an equilibrated 6×6), the three size thresholds, per-method counters, and `gpu-check --pairs/--chaos` and, until task H2 turned it round, `--policy` (§10.4 layer 3; the flag is gone and `--force-device` is what asks for 2b's forced behaviour) | | coarse **bit-identical on 2 934 052 of 2 934 122 hypotheses**, the other 70 one probe point away; stage 1 inside §10.2's rotation row on 7 of 8 sets and its translation row at the cloud on 8 of 8; stage 2 inside on 4 of 8, the tail separated by a control into ladder and kernel; end to end **the same used joins, groups and `evaluate.py` scores as the CPU on all seven collections**, two GPU runs byte-identical, CPU outputs unmoved | both risks were real: 29 accumulators do not fit one 16 KB reduction, and `f32` needed the shift *and* §7's composition correction *and* an equilibrated solve. The one this row did not name is the one that decided the phase: **the stage is 1.0× because the pipeline blocks ten threads on one queue**, which is 2d's |
| **2b, exit criterion, restated at the production policy (task H2), closed at eight of eight (task F)** | — | — | **Met, on all eight development sets, at the run level and on the kernel rows.** The criterion is the audit's §A.2.4: the device does R §5.2's coarse score and R §5.4's two stage-1 breakline rungs, R §5.6's stage 2 is the CPU's by policy (`icp::STAGE2_ON_DEVICE`), the translation row is read **at the cloud**, and the two run-level rows are what a placed pose is held to — identical used joins, identical groups, identical `evaluate.py` scores on both backends, and a worst final pose gap of 7.0e-2° / 2.9e-4 t, inside §10.2's `refine` row. `gpu-check --stage all --pairs 4` exits **0 on all eight** — terracotta, pot_A, pot_B, pot_C, pot_G, pot_H, `synthetic_20` and the slab — against 7 of 8 at task H2 and 1 of 8 when every rung was forced onto the device. **What closed pot_C** (V7-D1) is not a widened tolerance: every number in §10.2 is where it was. It is the third excusal of §10.4 layer 3, `crosscheck::boundary_batch`. Task F measured pot_C's one failing candidate of a thousand to the iteration: at its ninth of twenty a correspondence sits **1.3e-6 of the radius** from the boundary, inside the `f32` resolution of the clouds there (7e-6 of the radius), and the two executors put it on opposite sides — after which their sets differ by one point of 518 and their answers by 9.1e-2°. That is §A.2.2's *first* discontinuity, the correspondence set, which neither the control nor C2's one-ULP probe can reach, and everything else about the candidate is determined (control 3.2e-6°, `--chaos` clean, the CPU's own `f32` point loops within 2.3e-6°, the grid and the KD-tree agreeing on every pose either side reaches). | the audit predicted 8 of 8, task H2 measured 7 and named the candidate, and task F found what it was. It is **not** §6.7's stopping-rule discontinuity, which is what H2 read it as: both sides run all twenty iterations without converging, and the divergence is a single correspondence crossing the radius at iteration 9. The `bound` row bounds what the new excusal may cover — worst pair 2.0e-2 of 6.0e-2 over the eight sets — so an exclusion that grew to hide a regression would fail it |
...
| **phase 2 total** | | **9** | | |
| 7 | **Measure before the tiers** (audit §D.1, §D.2). For every accepted candidate of the forty runs of §10.4's layer 7: R §6's five scores, the margin to the second *placement* of the same pair (`candidates.rs`'s `SAME_PLACEMENT_T`), `determined`, stability under two independent redraws of `Pf` and `S`, the slide probe (restart the last fracture rung from ±0.5 t along the breakline tangent), the support count, and `evaluate.py`'s class for the join. One table, one note. The **same pass, preprocessing only**, computes §11's per-fragment `Features` — wall thickness, shell radius, fracture roughness, Lab colour where the vertices carry it, rim flag and rim diameter, axis fit and its residual — on pots A–H, `mixed_ABG`, `synthetic_170` and `mixed_all`, and reports each feature's same-object-vs-different-object AUC, the share of pairs a `k·MAD` veto would remove and the true adjacent pairs it would cost | **8 h** | the table exists, and the note states the strict thresholds, the margin `m` and the feature shortlist **chosen on it**, each with the false-join margin it buys | the measurement may separate nothing and the shortlist may come back empty — that is a result, not a failure, and it is cheaper to find here than inside step 8. Colour has no AUC to measure on any set that carries object ids (audit §C.4): the SfS++ OBJs have bare `v x y z` lines and both synthetic sets are one constant terracotta |
| 8 | **Tiers.** `Tier` on `Candidate`; a strict `Thresholds` set in `Params` beside R §6.5's; step 7's three probes run on **accepted candidates only**; the assembly built from **confirmed** joins alone; `report.md` gains Confirmed / Probable / Rejected sections and a per-fragment candidate index (audit §C.8a: the useful bench output is per fragment, not only per pair); `transforms.json` carries the tier | **10 h** | **zero false joins in the confirmed tier on 9 sets × 5 seeds** (8 until task S1 added `synthetic_mix3_24`) — every confirmed join classed `correct` by `evaluate.py`, with `wrong_pose`, `non_adjacent` and `cross_object` all 0 across the forty-five runs and `unscorable` ones listed — **and confirmed recall reported per set beside it** (60–80 % of the true joins is what audit §D.1 expects; that loss is the product). The terracotta's two joins confirmed at all five seeds — **met since task S3**, ten of ten slots. **Both halves hold on the development sets, and since task R1 the first half very nearly holds on `mixed_all` too**: 13 false confirmed joins over two seeds under S3's rule (task A2), **1 and no cross-object one** under R1's, and **0** with `--tier-agree-seeds 2` (§10.3). Outputs byte-identical to step 7's with `--tiers off` | the probes cost two resamples and two extra rungs per accepted candidate, which is a few per cent of a pair and has to stay that way. ~~If confirmed recall collapses below half the true joins the tier is not shippable and the thresholds go back to step 7's table rather than being widened to pass~~ — **the first half of that clause is spent and the second half is live** (task C, V9-D6). Step 7's own measurement is that **no** conjunction of the evidence reaches zero false joins at the recall this row assumed (M1: 4 478 976 searched), so a tier that keeps the prohibition cannot also keep the recall: the shipped one confirms **17.9 %** of the development sets' adjacent pairs (task R1, nine sets × seeds 0–4; S3's rule reached 23.2 %, M1's 16.9 %, and A1 measured **11.3 %** on its three acceptance collections) and, on the two collections task R1's own table contains, **24.4–25.2 %** of `synthetic_mix3_60`'s and **6.6–8.0 %** of `mixed_all`'s — against audit §D.1's expected 60–80 %. What ships instead of the recall is the **probable band**, which carries the rest of R §6.5's accepted joins to the bench with the test each one failed. What is still binding is the second half: **thresholds are never widened to pass a row.** Whether a confirmed band at this recall is what a museum wants is the open question §12's own step-12 entry hands on |
...
| 12 | **Final acceptance — done, task A1, 2026-09-10** (`notes/2026-09-10-a1-acceptance.md`). The **only** step that has matched a collection above 27 fragments. Six runs on the CPU, tiers and objects at their shipped defaults, previews and meshes off, `--memory-budget 4.83`: `synthetic_60`, `synthetic_170` and `mixed_all` at seeds 0 (cold cache) and 1 (warm), each under `/usr/bin/time -l`, each scored by `evaluate.py` through `quality_gate.py --score-only`, peak RSS per stage from §8's monitor. §10.3's three projected rows are **replaced by measurements** there. The museum's own acceptance set (audit §C.4) has not arrived and is still to be run the same way | **6 h** | **Wall and memory: met.** 2.1 / 18.5 / 30.3 min cold against 15 min / 2 h / 2 h — 7.0×, 6.5× and 4.0× of margin — and **2.0** / 18.4 / 30.3 min warm; worst peak RSS **2 715 MiB** (`/usr/bin/time -l`; 2 647 MiB on `report.json`'s own sampler), 44 % of §1's 6 GB row; the memory semaphore never blocked. **Quality: met on the two synthetic collections, failed on `mixed_all`.** 148 confirmed joins over the four synthetic runs, every one `correct`, cross-object **0** and purity **1.000** — including on `synthetic_170`, whose six intruder pieces make a cross-object join possible. On `mixed_all`, **10 of 51 confirmed joins over two seeds are false** and three of them are cross-object at seed 1 (purity 0.930), which breaches the prohibition. Confirmed recall 0.161–0.182 / 0.118–0.125 / 0.066–0.076; probable list 38–43 / 92–94 / **2 761–2 795**. A README section for museum users on reading the tiers is written | **The projection was close and the tier pass was the surprise.** E2's per-pair costs came in at +10.5 %, +36.2 % and −4.7 % at a hundred times the pair count and the pool returned 8.5× rather than 6.43× — the end-to-end **wall** is what held to 5–12 %, and it held because those two errors ran against each other (§10.3, V9-D11). But R §6.5 accepts 8 444 candidates on a real ten-pot collection and the tier probes every one, which is 580 s — a third of that run. §8's 4.4 GB BVH extrapolation was never tested: it assumes 34 M working-mesh faces and these collections have 4.7–9.0 M. **The failure `mixed_all` records is roadmap item 4's, not the tier's**: the Pot_I sherd of each impure group agrees with its Pot_E group to within the group's own MAD on every feature the cache carries, which is task M1 §4's AUC of 0.740 becoming a false join. Requiring `support ≥ 1` removes all ten and costs 124 of the 189 correct confirmed joins — measured, and deliberately not applied |
| 12b | **The rule chosen on a table that contains `mixed_all` — done, task R1, 2026-09-11** (`notes/2026-09-11-r1-rule-on-real.md`). Step 12a judged task S3's rule and found it confirms 13 false joins on the museum's own ten pots; a rule chosen on a table without that collection is a rule chosen without the evidence. Four large runs and no others: `mixed_all` and `synthetic_mix3_60` at seeds 0 and 1 with `--tiers on --resample-seeds 2 --measure`, so that a table of 171 rules is read **offline** from four dumps beside the 45 development ones; then two confirmation runs of `mixed_all` at seed 0, one at the shipped default and one with `--tier-agree-seeds 2` | **8 h** | **Quality: met on the coloured collection, and all but met on `mixed_all`.** The shipped rule confirms **41 correct joins and 1 false over two seeds, cross-object 0**, against S3's 62 and 13 with two cross-object, and **19 and 0** at seed 0 alone with group purity 1.000; `synthetic_mix3_60` 63 and 0; the 45 development runs **174 and 0** with the terracotta's ten slots held, against S3's 225 and 0. The remaining false join is named and measured: `Pot_I_Piece_07–Pot_I_Piece_11` at **1.064° and 0.570 t** against `evaluate.py`'s 0.5 t. **Wall: 39.5 min, 3.0× inside the 2 h gate**, and `--tier-agree-seeds 2` at 78.6 min, 1.5× inside it, with **zero** false confirmed joins | **No number in the rule moved, and that is the point.** The three conjuncts task R1 added are booleans over evidence task S3 already reported — both re-searches agreeing, the second placement being one R §6.5 refuses, the strict half holding on the two redraws — because task A2's finding was that a *threshold* fitted on one pot does not transfer, and §10.3's re-derivation says it again: the rival distance separates correct from false at AUC 0.776 on the development runs and **0.562** on `mixed_all`. The table does contain single-run rules that reach zero everywhere, and every one of them loses audit §E step 8's terracotta row; that is why the thing that reaches zero ships as a **mode** (`--tier-agree-seeds`) and not as the default |
| 12a | **Second acceptance — done, task A2, 2026-09-11** (`notes/2026-09-11-a2-acceptance.md`). Step 12 judged M1's confirmation rule; task S3 replaced it, so the step was re-run on the two collections that answer for it. Seven large runs on the CPU at the shipped defaults, `--memory-budget 4.83`, previews and meshes off: `synthetic_mix3_60` (task S1's three-vessel coloured collection, matched for the first time) and `mixed_all` at seeds 0 and 1, plus three paired runs with `--tier-research 0 --resample-seeds 0` that price the re-search. `synthetic_60` and `synthetic_170` were not re-run | **6 h** | **Wall and memory: met, with less margin.** 2.2 min and 42.3 min cold against 15 min and 2 h — 6.8× and **2.8×**, where A1 had 4.0× on the same collection — and a worst peak RSS of 1 827 MiB, 30 % of §1's 6 GB row. **Quality: met on `synthetic_mix3_60`, failed again on `mixed_all`.** 38 and 41 confirmed joins on the coloured collection, every one correct, cross-object 0, purity 1.000, and a probable band of 23 and 21 rows that is **100 % true joins**; its `## Objects` rows separate the three vessels by `frac_lab_a` (2.40–4.13 / 6.64–7.62 / 13.44–17.18) where `thick` overlaps them completely. On `mixed_all`, **62 correct confirmed joins against A1's 41 and 13 false against A1's 10** — recall 7.1 % → 10.8 %, tier precision 0.804 → 0.827, cross-object 3 → 2, purity 0.930 → 0.966, fragment accuracy +10 points — and the prohibition breached at both seeds | **The rival-distance conjunct does not generalise, and the run says so in one line.** S3 fitted `RIVAL_FAR_T = 5` on 89 false candidates of which 69 were `pot_H`; on `mixed_all` the false joins' second placements sit at a median of 15.85 t against the correct ones' 18.04 t — no separation, where the development sets gave 2.98 t against 19.83 t. The re-search, which S3 §9 proposed dropping, is the conjunct that *does* earn its place here: without it the band gains two false joins and both are **cross-object**. It costs 569 s, 43 % of the matching stage. `support ≥ 1` alone is still the only clean rule and still costs three quarters of the correct joins — measured, not applied |
