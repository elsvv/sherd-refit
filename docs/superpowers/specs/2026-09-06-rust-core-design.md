...
| `mixed_all` (12 612 pairs) | ≤ 2 h | ≤ 30 min | **run once, at the final acceptance** (task A1): **30.3 min** CPU cold, **30.3 min** warm — inside, 4.0×. **Re-measured under task S3's rule at the second acceptance** (task A2, 2026-09-11): see the A2 table below |
...
7. **Quality gate** (`tools/quality_gate.py`, task H4): the eight development sets — terracotta,
   ...
   seed sweep. **Measured on the shipped defaults: 45 runs in 16.5 min** on this machine
   (988.0 s, task A2) against the 25-minute budget the hand-over was made under. The set list grew
   ...
   * *Step 8, the terracotta.* "Both joins confirmed at seeds 0–4" is **ten** (seed, join) slots and
     the gate asks for ten. **This tree reaches ten** (2/2/2/2/2, task A2 §8). It reached nine from
