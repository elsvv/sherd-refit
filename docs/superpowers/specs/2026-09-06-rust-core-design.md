...
7. **Quality gate** (`tools/quality_gate.py`, task H4): the eight development sets — terracotta,
   ...
   seed sweep. **Measured on the shipped defaults: 40 runs in 10.0–10.3 min** on this machine —
   five runs of the same forty across tasks O1, G, A1, V8 and V9 measured 602.2, 610.9, 611.7,
   614.9 and 602.6 s, each one's own `meta.wall_total_s` in `quality.{md,json}` — against the
   25-minute budget the hand-over was made under. Cold caches, the work directory of a set reused
   ...
   tier pass**: the same forty runs under `--tiers off` take 412.2 s (task V8) and 459.8 s
   ...
   * The **gate** is what R §13 states as a *prohibition*: its last row — cross-object joins 0 and
     ...
     hold at all five seeds on this tree — **and the run still exits 1**, on the terracotta's
     *recall* row (nine of ten (seed, join) slots; §10.4 below). A red gate here is not a breached
     prohibition, and the run's last line says which of the two it is.
...
| 8 | **Tiers.** `Tier` on `Candidate`; a strict `Thresholds` set in `Params` beside R §6.5's; step 7's three probes run on **accepted candidates only**; the assembly built from **confirmed** joins alone; `report.md` gains Confirmed / Probable / Rejected sections and a per-fragment candidate index (audit §C.8a: the useful bench output is per fragment, not only per pair); `transforms.json` carries the tier | **10 h** | **zero false joins in the confirmed tier on 8 sets × 5 seeds** — every confirmed join classed `correct` by `evaluate.py`, with `wrong_pose`, `non_adjacent` and `cross_object` all 0 across the forty runs and `unscorable` ones listed — **and confirmed recall reported per set beside it** (60–80 % of the true joins is what audit §D.1 expects; that loss is the product). The terracotta's two joins confirmed. Outputs byte-identical to step 7's with `--tiers off` | the probes cost two resamples and two extra rungs per accepted candidate, which is a few per cent of a pair and has to stay that way. ~~If confirmed recall collapses below half the true joins the tier is not shippable and the thresholds go back to step 7's table rather than being widened to pass~~ — **the first half of that clause is spent and the second half is live** (task C, V9-D6). Step 7's own measurement is that **no** conjunction of the evidence reaches zero false joins at the recall this row assumed (M1: 4 478 976 searched), so a tier that keeps the prohibition cannot also keep the recall: the shipped one confirms **16.9 %** of the development sets' adjacent pairs and **11.3 %** of the acceptance sets' (§10.3, §10.4), against audit §D.1's expected 60–80 %. What ships instead of the recall is the **probable band**, which carries the rest of R §6.5's accepted joins to the bench with the test each one failed. What is still binding is the second half: **thresholds are never widened to pass a row.** Whether a confirmed band at this recall is what a museum wants is the open question §12's own step-12 entry hands on |
